...
function gameLoop(e) {
  ...
  const endGame = () => {
    clearCanvas();
    socket.close();
    fetch("https://localhost/tournament/end_game/", {
      method: "POST",
...
export function setupTournament() {
  ...
  next_game_button.addEventListener("click", function () {
    message.textContent = "";
    next_game_button.style.display = "none";
    socket = new WebSocket(`wss://localhost/ws/tournament/${roomName + "f"}/`);
  ...
  document
    ...
    .addEventListener("submit", function (event) {
      ...
      socket = new WebSocket(`wss://localhost/ws/tournament/${roomName}/`);
      ...
      fetch("https://localhost/tournament/create_room/", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          roomNumber: roomName,
  ...
  document
    ...
    .addEventListener("submit", function (event) {
      ...
      socket.onmessage = function (event) {
        ...
        if (data.players !== undefined) updatePlayerList(data.players);
      };

      fetch("https://localhost/tournament/join_room/", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          roomNumber: roomName,
