...
export function setupProfile() {
  ...
  if (!parse_cookie.is_uploadpp) img.src = "../default/pp.png";
  else img.src = "../default/pp2.png";
  ...
  else twofa_button.style.backgroundColor = "red"; // Butonun arka plan rengini kırmızı yap

  function changeImage() {
    let _cookie = document.cookie;
    let parse_cookie = parseJWT(_cookie);
    fetch("https://localhost/api/uploadpp/", {
      method: "POST",
      headers: {
      ...
      .then((data) => {
        let _cookie = document.cookie;
        let parse_cookie = parseJWT(_cookie);
        if (parse_cookie.is_uploadpp) img.src = "../default/pp2.png";
        else img.src = "../default/pp.png";
      })
  ...
  function activate2FA() {
    ...
    fetch("https://localhost/api/2faactive/", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
      ...
      .then((data) => {
        ...
        else twofa_button.style.backgroundColor = "green";
      })
      .catch((error) => console.log("Error:", error));
  }
  ...
  function makeEditable(field) {
    ...
    input.onkeydown = function (event) {
      if (event.key === "Enter") {
        ...
        fetch("https://localhost/api/updateuser/", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
